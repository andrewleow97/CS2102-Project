...
BEGIN
    ...
    ------- CREATE TRIGGER TO CHECK DATE OF UPDATE >= CURRENT_DATE ----------------
...
BEGIN
    ----------- CREATE TRIGGER TO ENSURE NEW_EID IS MAX(EID) + 1 BEFORE INSERT ------------------------
    --The unique employee ID and email address are automatically generated by the system.
    SELECT MAX(eid) into new_eid from Employee;
    new_eid := new_eid + 1;
    ...
    ------------ CREATE TRIGGER TO ENSURE BEFORE INSERT JUNIOR EID NOT EXIST IN BOOKER AND VICE VERSA ---------
    ------------ CREATE TRIGGER TO ENSURE BEFORE INSERT MANAGER EID NOT EXIST IN SENIOR AND VICE VERSA -----------
